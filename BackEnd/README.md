...
To build the final jar and optimize the mercotraceRetailer application for production, run:
...
## Database and Mercotrace UI (this repo)

PostgreSQL is expected on **`localhost:5433`** with superuser **`postgres`** (password set in `application-secret-samples.yml`, overridable via `SPRING_DATASOURCE_PASSWORD`). The app uses its **own** database **`mercotraceretailer`** (not `glow_business_board` / `glow_user`).

1. Create the database once (as `postgres`):
   ...
   If `CREATE DATABASE` fails because the DB already exists, you can skip this step.

2. Start the API: `./mvnw` (Windows: `mvnw.cmd`). Dev server port is **8090** (see `application-dev.yml`) to avoid clashing with other tools on 8080: [http://localhost:8090](http://localhost:8090).

3. The React app in `../` uses `.env.development` with `VITE_API_BASE_URL=http://localhost:8090`. Dev UI is typically [http://localhost:5173](http://localhost:5173). On the login screen, open **API login** and sign in with seed users **`admin` / `admin`** or **`user` / `user`**. The **Stores** page then loads `/api/stores` from this backend.
