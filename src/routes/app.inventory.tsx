              ...
              {rows.map(({ l, h, store }) => (
                <TableRow key={l.id}>
                  ...
                  <TableCell className="hidden md:table-cell">
                    {inr(l.costPrice)} / {inr(l.sellPrice)}
                  </TableCell>
                  <TableCell className="hidden md:table-cell">
                    {h.daysHeld}d held · exp {h.daysToExpiry}d
                  </TableCell>
                  <TableCell>{inr(h.effCost)}</TableCell>
                  <TableCell className={h.margin < 0 ? "text-destructive font-medium" : ""}>
                    {(h.margin * 100).toFixed(1)}%
                  </TableCell>
                  <TableCell>
                    <StatusBadge tone={h.tone}>
                      {h.tone === "green" ? "Healthy" : h.tone === "yellow" ? "At risk" : "Loss"}
                    </StatusBadge>
                  </TableCell>
                </TableRow>
              ))}
